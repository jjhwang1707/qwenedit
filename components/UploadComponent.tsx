...
export default function UploadComponent({ onUploadComplete }: { onUploadComplete: (id: string, originalUrl: string, analysis: any) => void }) {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      reader.onload = async () => {
        ...
        // 2. Call our Next.js Gemini API Route
        ...
        onUploadComplete(imageId, base64Image, result.data)
      }
    } catch (error) {
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center">
      <div className="mb-8 text-center">
        ...
        <p className="text-gray-500">Upload an image. Gemini Vision extracts the elements. Nanobanana edits them.</p>
      </div>
      <label className="flex flex-col items-center justify-center w-full max-w-xl h-64 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer bg-white hover:bg-gray-50 transition-colors shadow-sm">
        <div className="flex flex-col items-center justify-center pt-5 pb-6">
          ...
          {uploading && <p className="mt-4 text-blue-500 font-medium">Analyzing with Gemini Vision...</p>}
        </div>
